  ...
  <div class="home">
    ...
    <ul class="effect-list">
      <li v-for="(item, k) in effectList" :key="k">
        <button :class="{active: activeIndex == k}" @click="setNewEffect(item, k)">{{k}}</button>
      </li>
    </ul>
  </div>
</template>
  ...
  methods: {
    ...
    getMesh(fragmentShader) {
        ...
        for(let i = 0; i < 50; i++) {
          ...
          // let geometry = new THREE.CylinderGeometry(0.2, 0.2, 100, 16, 16, true);
        const geometry = new THREE.CircleGeometry(50, 64);
